...
def run_comparison(query: str, max_steps: int):
    ...
    Uses Gradio's generator pattern for real-time chart and step log updates.

    Yields: (reward_chart, trad_log_html, oe_log_html) after each agent step.
    """
    if not query.strip():
        yield (
            empty_chart("Reward Comparison"),
    ...
    # Final summary callouts appended below each agent's step log
    ...
    trad_html_blocks.append(agent_summary(
    ...
    oe_html_blocks.append(agent_summary(
...
def run_race(query: str, max_steps: int):
    ...
    All 3 agents connect to the same Docker container via GenericEnvClient.
    OpenEnv's SUPPORTS_CONCURRENT_SESSIONS=True gives each agent its own
    isolated episode state despite sharing one container process.

    Yields an HTML scoreboard updated after each agent step, followed by
    a summary card with narrative, scores, and session stats.
    """
    if not query.strip():
    ...
    # Append post-race summary card below the final scoreboard
